...
· Idea de Cesar: mientras él produce el arte (texturas y deco del pipeline de
  la R125+), crear un sandbox de imagen fuera del juego — «el prólogo no es
  el lugar óptimo para tomar decisiones de estilo» — con acceso dev para
  colocar fogatas, máquinas y elementos.
· `docs/PLAN_GALERIA_DE_ESTILO.md` + esquema `docs/ref/esquema_galeria.png`
  (9 áreas: cueva íntima con spawn, patio de fuego, la nave, pared de juntas,
  el pozo, poza y humedad, el vano, pendientes y silueta, el terrario;
  conectadas por corredores para juzgar JUGANDO a 80 celdas). Ubicación
  técnica sin romper nada: flag `ModoGaleria` (patrón Fundación, regla 59),
  misma escena/sim/piel/muñeco, plano propio `GaleriaLevelBuilder`, menú
  «7. GALERÍA DE ESTILO», catálogo IMGUI hermano de DevPalette. Metas:
  1 EL LIENZO (plano + colocables existentes + teletransporte + botón de
  capturas de las 9 áreas), 2 EL PROBADOR (hot-load de texturas de Cesar
  desde carpeta Galeria/ con tecla R, conmutador A/B, perillas de la piel),
  3 EL BANCO DE LUZ (2D Renderer + luces con causa: el experimento de la
  V2 §3 corre entero dentro). Regla: nada de la Galería migra al juego sin
  su propia ronda. Pendiente: cruzar el esquema con el mini-mapa de Cesar y
  disparar la Meta 1.
