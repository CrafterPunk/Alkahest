...
namespace Alkahest.Game
{
    ...
    public sealed class LabDiario : MonoBehaviour
    {
        ...
        private long _vidrioAnterior, _carbonAnterior, _plantasAnterior, _goteosAnterior, _cocidoAnterior;
        ...
        private void Update()
        {
            ...
            if (_jugador != null)
            {
                _distanciaCeldas += Vector3.Distance(_jugador.position, _posAnterior) / SimRenderer.CellWorldSize;
        ...
        private void Abrir()
        {
            string carpeta = Path.GetFullPath(Path.Combine(Application.dataPath, "..", "Laboratorio", "h7"));
            Directory.CreateDirectory(carpeta);
            var ahora = System.DateTime.Now;
            _ruta = Path.Combine(carpeta, "sesion_" + ahora.ToString("yyyy-MM-dd_HHmm", CultureInfo.InvariantCulture) + ".md");
            ...
            File.WriteAllText(_ruta, cab.ToString());
        ...
        private void Anotar(string texto)
        {
            ...
            File.AppendAllText(_ruta, _sb.ToString());
        }
        ...
        private void Marcar(string clase, string etiqueta)
        {
            ...
            _marcasTotales++;
            string nombre = "h7_" + _marcasTotales.ToString("00") + "_" + clase + "_" + Reloj().Replace(":", "") + ".png";
            string ruta = Path.GetFullPath(Path.Combine(Application.dataPath, "..", "Laboratorio", "h7", nombre));
        ...
        private void Muestrear()
        {
            ...
            if (!_hitoGoteo && st.LabGoteos > 0) { _hitoGoteo = true; Anotar("**PRIMER GOTEO** — el alambique destila"); }
            if (!_hitoCocido && st.LabCocido > 0) { _hitoCocido = true; Anotar("**PRIMERA ARCILLA COCIDA** — hay calor domesticado"); }
            if (!_hitoFuego && st.LabCombustibleQuemado > 0) { _hitoFuego = true; Anotar("**PRIMER FUEGO** — algo arde"); }
            if (!_hitoCarbon && st.LabCarbonizado > 0) { _hitoCarbon = true; Anotar("**PRIMER CARBÓN** — hay carbonera"); }
            if (!_hitoVidrio && st.LabVidrio > 0) { _hitoVidrio = true; Anotar("**PRIMER VIDRIO** — hay horno"); }
            if (!_hitoPlanta && st.LabPlantasNacidas > 0) { _hitoPlanta = true; Anotar("**PRIMERA PLANTA** — el huerto germina"); }
